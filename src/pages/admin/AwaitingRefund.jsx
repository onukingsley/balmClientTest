...
export default function AwaitingRefund() {
    ...
    const {adminOrders,updateRefundOrder,awaitingRefund} = AdminOrderStore()

    const [filteredOrder, setFilteredOrder] = useState(orders)


    useEffect(()=>{
        let ord = awaitingRefund
        ...
        if (searchQuery){
            Object.keys(ord).map((key)=>{
                ...
                if (rer){
                    filteredArray = {...filteredArray ,[key] : awaitingRefund[key]}
                }
                ord = filteredArray
        ...
        if (!ord){
            ...
            setFilteredOrder(awaitingRefund)
    ...
    },[searchQuery,statusFilter,awaitingRefund])
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-serif text-nude-900">Refund Orders</h1>
                    <p className="text-nude-600 mt-1">Cancelled Orders awaiting Refund</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                {Object.keys(filteredOrder).length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <p className="text-nude-600">No awaiting-Refund orders found</p>
                        </div>
                    )
                    : (
                        <div className="overflow-x-auto">
                            <table className="w-full min-w-[700px]">   {/* Added min-w to prevent too much squeezing */}
                                <thead className="bg-nude-50 sticky top-0 z-10">
                                <tr>
            ...
            <span className="font-medium text-nude-900">
              ₦{parseInt(filteredOrder[key][0]?.total_price || 0).toLocaleString()}
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Dialog>
                                                    ...
                                                    <DialogContent className="bg-white w-[95vw] max-w-lg sm:max-w-xl mx-auto max-h-[92vh] overflow-hidden p-0 flex flex-col rounded-2xl">

                                                        {/* Header */}
                                                        <DialogHeader className="px-6 py-4 border-b border-nude-100 flex-shrink-0">
                                                            <DialogTitle className="text-lg font-semibold">Order Details - {key}</DialogTitle>
                                                        </DialogHeader>
                                                            ...
                                                            <div className="space-y-6">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    {filteredOrder[key]?.length > 0 ? (
                                                                        ...
                                                                        <p className="text-nude-400">No items</p>
                                                                    )}
                                                                </div>
                            ...
                            ₦{parseInt(filteredOrder[key]?.[0]?.total_price || 0).toLocaleString()}
                                                                ...
                                                                <div className="border-t border-nude-100 pt-4">
                                                                    ...
                                                                    <div className="space-y-2 text-sm">
                                                                        <div className="flex gap-x-3">
                                                                            <span className="text-nude-600 w-24">Name:</span>
                                                                            <span className="text-nude-700">{filteredOrder[key]?.[0]?.user?.name}</span>
                                                                        </div>
                                                                        <div className="flex gap-x-3">
                                                                            <span className="text-nude-600 w-24">Email:</span>
                                                                            <span className="text-nude-700">{filteredOrder[key]?.[0]?.user?.email}</span>
                                                                        </div>
                                                                        <div className="flex gap-x-3">
                                                                            <span className="text-nude-600 w-24">Phone:</span>
                                                                            <span className="text-nude-700">{filteredOrder[key]?.[0]?.user?.phone_number}</span>
                                                                        </div>
                                                                        <div className="flex gap-x-3">
                                                                            <span className="text-nude-600 w-24">Account No:</span>
                                                                            <span className="text-nude-700">{filteredOrder[key]?.[0]?.user?.account_number}</span>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                ...
                                                {/* AlertDialog for Refund - Keep as is or improve similarly */}
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent className="bg-white max-w-md">
                                                        ...
                                                        <AlertDialogDescription>
                                                            Are you sure you want to process refund for this order?
                                                        </AlertDialogDescription>
                                                        <AlertDialogFooter>
