...
export default function Cancelled() {
    ...
    const {adminOrders,adminCancelledOrders, totalRevenue} = AdminOrderStore()
    ...
    useEffect(()=>{
        ...
        if (searchQuery){
            Object.keys(ord).map((key)=>{
                ...
                if (rer){
                    filteredArray = {...filteredArray ,[key] : adminCancelledOrders[key]}
                }
                ord = filteredArray
        ...
        if (!ord){
            ...
            setFilteredOrder(adminCancelledOrders)
    ...
    },[searchQuery,adminCancelledOrders])
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-serif text-nude-900">Cancelled Orders</h1>
                    <p className="text-nude-600 mt-1">View cancelled Orders</p>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <table className="w-full">
                        ...
                        {Object.keys(filteredOrder).map((key) => (
                            <tr key={key} className="hover:bg-cream-50">
                                ...
                                <td className="px-6 py-4">
                                    <Badge className="bg-red-100 text-sage-700">
                                ...
                                <td className="px-6 py-4">
                                    <Dialog>
                                        ...
                                        <DialogContent
                                            className="bg-white w-full max-w-lg mx-auto
               max-h-[90vh] overflow-hidden
               sm:max-w-lg md:max-w-xl
               p-0 flex flex-col"
                                        >
                                            <DialogHeader className="px-6 py-4 border-b border-nude-100">
                                                        ...
                                                        {filteredOrder[key]?.length > 0 ? (
                                                            filteredOrder[key].map((item, idx) => (
                                                                <div key={idx} className="flex justify-between py-3 border-b border-nude-100 last:border-b-0">
                                                                    ...
                                                                    <span className="text-nude-600 font-medium whitespace-nowrap">
                  x{item.quantity}
                </span>
                                                                </div>
                                                            ))
                                                        ) : (
                                                            <p className="text-nude-500 italic">No items found</p>
                                                        )}
                                                    </div>
              ...
              ₦{parseInt(filteredOrder[key]?.[0]?.total_price || 0).toLocaleString()}
                                            ...
                                            {/* Optional Footer */}
                                            <div className="p-4 border-t border-nude-100 flex-shrink-0">
                                                <Button
