...
export default function Brand() {
    const {brand,removeBrand} = productStore()
    ...
    axiosClient.post("/deleteBrand",payLoad)
        .then(({data})=>{
            console.log(data)
            removeBrand(productToDelete.id)
    ...
    return (
        <div>
            {!filteredBrand? (<div className="text-center py-12">
                        <Package className="h-12 w-12 text-nude-300 mx-auto mb-4" />
                        <p className="text-nude-600">No brand found</p>
                    </div>
                ):
                ...
                (  <div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                        <div>
                            <h1 className="text-3xl font-serif text-nude-900">Brand</h1>
                            <p className="text-nude-600 mt-1">Manage your brand catalog</p>
                        </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                        {[
                            { label: 'Brand', value: brand.length },
                        ].map((stat) => (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredBrand.length > 0  &&  filteredBrand?.map((brand) => (
                            ...
                            >
                                <div className="relative aspect-square bg-nude-100">
                                    <img
                                        src={`${import.meta.env.VITE_API_URL}/storage/${brand.brand_image}`}
                                ...
                                <div className="p-4">
                                    <span className="text-xs text-nude-500 uppercase">{brand.product.length} products</span>
                                    <h3 className="font-medium text-nude-900 mt-1">{brand.title}</h3>
                    ...
                    ${product.originalPrice}
                                    ...
                                    <div className="flex items-center gap-2 mt-4">
                                        <Link to={`/products?brand=${brand.title}`} className="flex-1">
